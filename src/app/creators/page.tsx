...
const creator = {
  ...
  status: 'ACTIVE',
  socials: [
    { platform: 'TikTok', handle: '@lisavandenberg', followers: '124K', color: 'bg-gray-900' },
...
export default function CreatorsPage() {
  ...
  return (
    <div>
      {/* Breadcrumb */}
      <div className="mb-6">
        <a href="/" className="text-sm text-accent-600 hover:text-accent-700 inline-flex items-center gap-1">
      ...
      <div className="bg-white rounded-lg border border-gray-100 p-6 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-5">
          <div className="w-16 h-16 rounded-full bg-accent-500 flex items-center justify-center text-white text-xl font-semibold shrink-0">{creator.initialen}</div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-xl font-semibold text-gray-900">{creator.naam}</h1>
              <Badge status={creator.status} />
            </div>
            <p className="text-sm text-gray-500 mt-0.5">{creator.bedrijfsnaam}</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button className="inline-flex items-center gap-2 px-4 py-2 bg-accent-600 text-white rounded-lg text-sm font-medium hover:bg-accent-700 transition">
              <Pencil className="w-4 h-4" /> Bewerken
            </button>
            <button className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <KpiCard label="Cash collected" value={eur(creator.kpis.cashCollected)} caption={`Openstaand: ${eur(creator.kpis.openstaand)}`} captionColor="warning" />
        <KpiCard label="Leads gegenereerd" value={creator.kpis.leads} caption="via persoonlijke quiz-link" />
        <KpiCard label="LTV / LTV:CAC" value={eur(creator.kpis.ltv)} caption={`Ratio: ${creator.kpis.ltvCac}`} captionColor="success" />
      </div>
          ...
          <dl className="space-y-3 text-sm">
            {[
              ...
              <div key={label} className="flex justify-between">
                ...
                <dd className={label === 'E-mail' ? 'text-accent-600' : 'text-gray-900 font-medium'}>{value}</dd>
              </div>
            ))}
          </dl>
          ...
          <div className="mt-5 pt-4 border-t border-gray-100">
            ...
            <div className="space-y-2">
              {creator.socials.map(s => (
                <div key={s.platform} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 ${s.color} rounded flex items-center justify-center`}>
                      <span className="text-white text-[10px] font-semibold">{s.platform.slice(0, 2).toUpperCase()}</span>
                    </div>
                    <span className="text-sm text-gray-700">{s.handle}</span>
                  </div>
                  <span className="text-xs font-medium text-gray-500">{s.followers}</span>
                </div>
              ))}
            </div>
            <button className="mt-3 text-xs text-accent-600 hover:text-accent-700 font-medium inline-flex items-center gap-1">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-lg border border-gray-100 p-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-gray-500">op {creator.contract.datum}</span>
              </div>
              <div className="text-sm text-gray-500">
                Geldig t/m: <span className="text-gray-700 font-medium">{creator.contract.verloop}</span>
              </div>
          ...
          <div className="bg-white rounded-lg border border-gray-100 p-6">
            ...
            <div className="flex items-center gap-3 bg-gray-50 rounded-lg px-4 py-3 mb-4">
              ...
              <code className="text-sm text-accent-600 font-medium flex-1 truncate">{creator.quizLink}</code>
              <button
                onClick={copyLink}
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition shrink-0 ${copied ? 'bg-emerald-500 text-white' : 'bg-accent-600 text-white hover:bg-accent-700'}`}
              >
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-gray-50 rounded-lg p-3 text-center">
                <div className="text-lg font-semibold text-gray-900">{creator.quizStats.klikken.toLocaleString()}</div>
                <div className="text-xs text-gray-500">Klikken</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-3 text-center">
                <div className="text-lg font-semibold text-gray-900">{creator.quizStats.leads}</div>
                <div className="text-xs text-gray-500">Leads</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-3 text-center">
                <div className="text-lg font-semibold text-emerald-600">{creator.quizStats.conversie}</div>
          ...
          {creator.omzetPerMaand.map((v, i) => {
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full bg-accent-600 rounded-t-md transition-all" style={{ height: `${pct}%` }} />
                <span className="text-[10px] text-gray-400">{creator.months[i]}</span>
              </div>
            )
          })}
        </div>
                ...
                <>
                  ...
                  >
                    <td className="px-6 py-3">
                      <ExternalLink className={`w-4 h-4 text-gray-300 transition-transform ${expandedLead === i ? 'rotate-90' : ''}`} />
                    </td>
                    <td className="px-4 py-3 text-gray-500">{lead.datum}</td>
                    <td className="px-4 py-3 font-medium text-gray-900">{lead.naam}</td>
                    <td className="px-4 py-3 text-gray-600">{lead.interesse}</td>
                      ...
                      <td colSpan={5} className="px-6 py-0">
                        <div className="bg-gray-50 rounded-lg p-4 my-2 ml-6">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                            <div><span className="text-gray-500">Leeftijd:</span> <span className="text-gray-800 font-medium">{lead.leeftijd} jaar</span></div>
                            <div><span className="text-gray-500">Interesse:</span> <span className="text-gray-800 font-medium">{lead.interesse}</span></div>
                            <div><span className="text-gray-500">Budget:</span> <span className="text-gray-800 font-medium">{lead.budget}</span></div>
                          </div>
                          <div className="mt-2 text-sm">
                            <span className="text-gray-500">Probleem/wens:</span>
                            <p className="text-gray-800 mt-0.5">{lead.probleem}</p>
                          </div>
                        </div>
                      </td>
                    </tr>
