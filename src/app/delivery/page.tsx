...
export default function DeliveryOverview() {
  ...
  if (loading || !stats) return <div className="flex items-center justify-center h-64"><div className="text-slate-400">Laden...</div></div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Delivery Overview</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <Link href="/delivery/werklijst" className="bg-white rounded-xl shadow-sm border border-slate-100 p-5 hover:shadow-md transition group flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center"><ListChecks className="w-5 h-5 text-brand-600" /></div>
          <div><h3 className="font-semibold text-slate-900 group-hover:text-brand-600 text-sm">Werklijst</h3><p className="text-xs text-slate-500">Inbox, huiswerk nakijken, check-ins</p></div>
        </Link>
        <Link href="/delivery/crm" className="bg-white rounded-xl shadow-sm border border-slate-100 p-5 hover:shadow-md transition group flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center"><Columns3 className="w-5 h-5 text-blue-600" /></div>
          <div><h3 className="font-semibold text-slate-900 group-hover:text-brand-600 text-sm">CRM</h3><p className="text-xs text-slate-500">Fase board per coach</p></div>
        </Link>
        <Link href="/delivery/backfill" className="bg-white rounded-xl shadow-sm border border-slate-100 p-5 hover:shadow-md transition group flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-amber-100 flex items-center justify-center"><FileEdit className="w-5 h-5 text-amber-600" /></div>
          <div><h3 className="font-semibold text-slate-900 group-hover:text-brand-600 text-sm">Backfill</h3><p className="text-xs text-slate-500">{incomplete} studenten incompleet</p></div>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="space-y-3">
            {coaches.map(c => {
              ...
              return (
                <Link key={c.id} href={`/delivery/crm?coach=${c.id}`} className="block bg-white rounded-xl shadow-sm border border-slate-100 p-5 hover:shadow-md transition">
            ...
            {coaches.length === 0 && (
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-8 text-center">
                ...
                <p className="text-sm text-slate-500">Nog geen coaches.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            {phaseData.map(p => {
              ...
              return (
                <div key={p.key} className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1 bg-slate-100 rounded-full h-2"><div className={`${p.color} h-2 rounded-full`} style={{ width: `${pctPhase}%` }} /></div>
                  <span className="text-xs font-medium text-slate-700 w-8 text-right">{p.count}</span>
                </div>
              )
            })}
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Verdienmodel</h3>
            {[
              ...
              { label: 'Geen', count: stats.total - stats.htc - stats.va - stats.as, color: 'bg-slate-300' },
            ].map(v => {
              const pctVm = stats.total > 0 ? (v.count / stats.total) * 100 : 0
              return (
                <div key={v.label} className="flex items-center gap-3 mb-2">
                  <span className="text-xs text-slate-500 w-14">{v.label}</span>
                  <div className="flex-1 bg-slate-100 rounded-full h-2"><div className={`${v.color} h-2 rounded-full`} style={{ width: `${pctVm}%` }} /></div>
                  <span className="text-xs font-medium text-slate-700 w-8 text-right">{v.count}</span>
                </div>
              )
            })}
          </div>
